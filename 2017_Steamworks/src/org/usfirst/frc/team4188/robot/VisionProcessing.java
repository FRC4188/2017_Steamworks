...
//if(!vision.filterContoursOutput.isEmpty() && vision.filterContoursOutput.size() >= 2 && Imgproc.boundingRect(vision.filterContoursOutput.get(0)).area()>5 && Imgproc.boundingRect(vision.filterContoursOutput.get(1)).area()>5) {
	...
	centerX = new double[]{r1.x, r.x};
	Imgcodecs.imwrite("output.png", Robot.getMat());
...
//Looking for the 2 blocks to actually start trig
	SmartDashboard.putBoolean("Area is In Range",(!vision.filterContoursOutput.isEmpty() && vision.filterContoursOutput.size() >= 2 && Imgproc.boundingRect(vision.filterContoursOutput.get(0)).area()>5 && Imgproc.boundingRect(vision.filterContoursOutput.get(1)).area()>5)
);
if(centerX.length == 2){
